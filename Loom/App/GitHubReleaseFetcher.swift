...
enum GitHubReleaseFetcher {
    ...
    private static func validateBundle(at appURL: URL, expectedVersion: String) throws {
        guard appURL.lastPathComponent == "Loom.app" else {
        ...
        guard info["CFBundleIdentifier"] as? String == "com.chasesims.Loom" else {
            throw FetcherError.invalidBundle("wrong bundle identifier")
        }
        guard info["CFBundleName"] as? String == "Loom",
              info["CFBundleDisplayName"] as? String == "Loom" else {
            throw FetcherError.invalidBundle("wrong display name")
        }
        guard info["CFBundleExecutable"] as? String == "Loom" else {
