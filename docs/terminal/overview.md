...
Loom saves terminal output locally as raw ANSI transcripts under
...
Loom also inserts a trailing space after the argument. Copied image files from Finder reuse the original file path. Direct clipboard images, such as screenshots, are encoded as PNG files under `~/Library/Application Support/Loom/Clipboard Images/` in the Loom build. Loom never presses Return for this path; the command stays editable so you can review it before running it.
