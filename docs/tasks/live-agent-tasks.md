...
- **Codex** rollout files are left untouched because they hold conversation history. Loom records a dismissal timestamp keyed to the product/model/session and hides the group until the rollout file's mtime advances past that mark. An active Codex session reappears after its next event; a stuck session stays cleared.
